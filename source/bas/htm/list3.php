...
class bas_htm_list3{
	...
	public function printme(){
		...
		if ($this->dataset->errormsg){
			...
			echo "<div class=\"tableContainer\" id=\"list_{$this->tabledef->id}\">\n"; 
			echo "<div class=\"scrollTableView\"><div class=\"tableView\">\n"; 
			//echo "<table>\n";
			
			
			//TODO: Rehacer los templates de la cabecera y pie.
			//NEW: ahora getcols() debe devolver todas las columnas, no solo las seleccionadas, ya que la selección la realiza el usuario.
			$cols=array();
			...
			if ($this->tabledef->hashead){
				...
				foreach($cols as $col) {
					...
					*/
					echo "<div class=\"tableCell\" id=\"col_{$col['id']}\">$data</div>"; //echo "<th$tdmod>$data</th>\n";
					$icol++;
				}
				echo "</div>\n"; //echo "</tr>\n";
				echo "</div>\n"; //echo "</thead>\n";
			...
			while ($rec){
				...
				foreach($cols as $col) {
					...
					echo "<div class=\"tableCell\" id=\"col_{$col['id']}\"$tdmod>$data</div>";//echo "<td$tdmod>$data</td>\n";
					$ncol++;
				}
				echo "</div>\n"; //echo "</tr>\n";
			...
			if ($this->tabledef->hasfoot){
				...
				foreach($cols as $col) {
					...
					echo "<div class=\"tableCell\" id=\"col_{$col['id']}\">$data</div>"; //echo "<td$tdmod>$data</td>\n";
					$icol++;
				}
				echo "</div>\n"; //echo "</tr>\n";
				echo "</div>\n"; //echo "</tfoot>\n";
				...
				$icol = 0;
				foreach($cols as $col) {
					echo "<div class=\"tableCell\" id=\"{$col['id']}\"></div>"; //echo "<td$tdmod>$data</td>\n";
					$icol++;
				}
				echo "</div>\n"; //echo "</tr>\n";
				echo "</div>\n"; //echo "</tfoot>\n";
